...
export const applyCorsPolicy = (
  ...
  if (!isAllowedOrigin) {
    response
      .status(403)
      .json(buildErrorResponse("CORS_FORBIDDEN", "Origin is not allowed"));
    return;
  }
...
export const handleMalformedJson = (
  ...
  if (error instanceof SyntaxError && "body" in error) {
    response
      .status(400)
      .json(buildErrorResponse("INVALID_JSON", "Request body must be valid JSON"));
    return;
  }
...
export const createRateLimiter = (
  ...
  return (request: Request, response: Response, next: NextFunction): void => {
    ...
    if (bucket.count > options.maxRequests) {
      response
        .status(429)
        .json(buildErrorResponse("RATE_LIMITED", "Too many requests. Please try again later."));
      return;
    }
